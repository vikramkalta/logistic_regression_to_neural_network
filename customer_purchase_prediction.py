...
class CustomerPurchasePrediction:
    ...
    def generate_realistic_data(self, n_samples=1000):
        ...
        - Device_type: Mobile, Desktop, Tablet (encoded as 0, 1, 2)
        - Campaign_type: Email, Social, Search, Direct (encoded as 0, 1, 2, 3)
        """
        ...
        device_types = np.random.choice([0, 1, 2], n_samples, p=[0.6, 0.3, 0.1])  # Mobile, Desktop, Tablet
        campaign_types = np.random.choice([0, 1, 2, 3], n_samples, p=[0.4, 0.3, 0.2, 0.1])  # Email, Social, Search, Direct
        ...
        purchase_probability = (
            ...
            0.1 * (device_types == 1) +  # Desktop users
    ...
    def explore_data(self, data):
        ...
        purchased = data[data['purchased'] == 1]['age']
        not_purchased = data[data['purchased'] == 0]['age']
        axes[0,0].hist([not_purchased, purchased], bins=20, alpha=0.7, label=['No Purchase', 'Purchase'])
        axes[0,0].set_title('Age Distribution by Purchase')
        axes[0,0].set_xlabel('Age')
        axes[0,0].legend()
        
        # Income distribution by purchase
        purchased_income = data[data['purchased'] == 1]['income']
        not_purchased_income = data[data['purchased'] == 0]['income']
        axes[0,1].hist([not_purchased_income, purchased_income], bins=20, alpha=0.7, label=['No Purchase', 'Purchase'])
        axes[0,1].set_title('Income Distribution by Purchase')
        axes[0,1].set_xlabel('Income')
        axes[0,1].legend()
        
        # Time on site by purchase
        purchase_groups = [data[data['purchased'] == 0]['time_on_site'], data[data['purchased'] == 1]['time_on_site']]
        axes[0,2].boxplot(purchase_groups, labels=['No Purchase', 'Purchase'])
        ...
        bars = axes[1,0].bar(range(len(purchase_by_device)), purchase_by_device.values)
        ...
        axes[1,0].set_xticklabels(device_names)
        
        # Campaign type purchase rates
        campaign_names = ['Email', 'Social', 'Search', 'Direct']
        purchase_by_campaign = data.groupby('campaign_type')['purchased'].mean()
        bars = axes[1,1].bar(range(len(purchase_by_campaign)), purchase_by_campaign.values)
        axes[1,1].set_title('Purchase Rate by Campaign Type')
        axes[1,1].set_xlabel('Campaign Type')
        axes[1,1].set_ylabel('Purchase Rate')
        axes[1,1].set_xticks(range(len(campaign_names)))
        axes[1,1].set_xticklabels(campaign_names)
    ...
    def evaluate_model(self, X_train, X_test, y_train, y_test):
        ...
        y_pos = range(len(feature_df))
        axes[2].barh(y_pos, feature_df['importance'])
        axes[2].set_yticks(y_pos)
        axes[2].set_yticklabels(feature_df['feature'])
...
def main():
    ...
    # Create a sample customer (age=35, income=65000, time_on_site=15, previous_purchases=3, email_opens=8, device_type=1 (Desktop), campaign_type=0 (Email))
    new_customer = np.array([[35, 65000, 15, 3, 8, 1, 0]])
    ...
    print(f"  • Customer profile: 35 years old, $65,000 income, 15 min on site, 3 previous purchases, 8 email opens, Desktop user, Email campaign")
